...
import re
import pandas as pd  # เพิ่มบรรทัดนี้เพื่อแก้ NameError
...
if mode == "📈 วิเคราะห์หุ้น":
    ...
    if st.button("🪄 สั่ง AI วิเคราะห์ราคาหุ้น"):
        try:
            ...
            st.error("ไม่สามารถดึงข้อมูลหุ้นได้ โปรดตรวจสอบการเชื่อมต่อ")
    ...
    if raw_input:
        ...
        if tops and bots:
            ...
            if st.button("🔮 สั่ง AI คำนวณและวิเคราะห์ 50 คู่ฮิต"):
                ...
                # --- ส่วนสถิติ 50 อันดับ (แสดงใต้ตาราง) ---
                st.divider()
                st.subheader("📈 วิเคราะห์ 50 อันดับคู่เลขที่ออกบ่อยที่สุด (จากสถิติที่วาง)")
                
                col_left, col_right = st.columns(2)
                
                with col_left:
                    st.markdown("#### ⭐ **Top 50 คู่บน** (บ่อยสุด)")
                    df_top = pd.DataFrame(top_counts, columns=['เลขคู่บน', 'จำนวนครั้ง'])
                    st.dataframe(df_top, use_container_width=True, height=400)
                
                with col_right:
                    st.markdown("#### ⭐ **Top 50 คู่ล่าง** (บ่อยสุด)")
                    df_bot = pd.DataFrame(bot_counts, columns=['เลขคู่ล่าง', 'จำนวนครั้ง'])
                    st.dataframe(df_bot, use_container_width=True, height=400)
        else:
            st.info("💡 กรุณาวางสถิติในช่องด้านบนเพื่อให้ระบบเริ่มวิเคราะห์")
