...
# --- โหมดที่ 2: วิเคราะห์ยี่กี ---
    ...
    st.title("🎯 AI วิเคราะห์ยี่กี + สถิติคู่เลขยอดฮิต")
    ...
    if raw_input:
        ...
        if tops and bots:
            ...
            st.success(f"🔍 ข้อมูลล่าสุดจากสถิติที่วาง: บน {last_top} / ล่าง {last_bot}")
            ...
            if st.button("🔮 สั่ง AI คำนวณและวิเคราะห์คู่เลข"):
                ...
                # --- ส่วนสถิติวิเคราะห์คู่เลข (แสดงใต้ตาราง) ---
                st.divider()
                st.subheader("📈 วิเคราะห์คู่เลขที่ออกบ่อยที่สุด (จากสถิติที่วาง)")
                col_a, col_b = st.columns(2)
                
                with col_a:
                    st.success("⭐ 5 อันดับ **คู่บน** ยอดฮิต")
                    for pair, count in top_counts:
                        st.write(f"👉 เลข **{pair}** : ออกไป {count} ครั้ง")
                
                with col_b:
                    st.warning("⭐ 5 อันดับ **คู่ล่าง** ยอดฮิต")
                    for pair, count in bot_counts:
                        st.write(f"👉 เลข **{pair}** : ออกไป {count} ครั้ง")
        else:
            st.info("💡 กรุณาวางสถิติในช่องด้านบนเพื่อให้ AI เริ่มทำงาน")
